...
module.exports = function(grunt) {
	    ...
	    .initConfig({
	      ...
	      sass : {
		      dist : {
		        ...
		        files : {
			        '<%= yeoman.app %>/styles/style.css' : '<%= yeoman.app %>/styles/app.scss'
		        },
		      }
	      }
	    });
	...
	grunt.registerTask('test', [ 'clean:server', 'concurrent:test',
	    'autoprefixer', 'connect:test', 'karma:unit' ]);

	grunt.registerTask('test:e2e', [ 'clean:server', 'concurrent:test',
	    'autoprefixer', 'connect:test', 'karma:e2e' ]);

	grunt.registerTask('build', [ 'clean:dist', 'bower-install', 'useminPrepare',
