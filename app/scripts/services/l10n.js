...
angular.module('mahjongLearnApp').service('L10n', function($rootScope, $resource) {
    ...
    this.getTexts = function() {
        return (storageAPI && storageAPI[prefixes.textKeys] ? JSON.parse(storageAPI[prefixes.textKeys]) : null) || texts;
    };
    ...
    function retrieveTextsFromFile() {
        return $resource('/static/texts.json').get(function(staticTexts) {
            texts = staticTexts.texts;
            $rootScope.texts = texts;
            storageAPI[prefixes.textKeys] = JSON.stringify(texts);
        },
        function(error) {
            throw new ReferenceError('The texts for language couldn\'t be loaded by the application.');
        });
    }
    ...
    $rootScope.getTexts = function() {
        if (!getTexts() || !storageAPI.getItem(prefixes.textKeys)) {
            ...
            return getTexts()|| (storageAPI && storageAPI[prefixes.textKeys] ? JSON.parse(storageAPI.getItem(prefixes.textKeys)) : null);
        }
    };

    $rootScope.setLanguage = function(language) {
        var newOverallLanguage;
        ...
        $rootScope.$broadcast('languageChange', {newLanguage: newOverallLanguage});
    };

    $rootScope.getLanguage = function() {
        return (storageAPI ? storageAPI.getItem(prefixes.language) : null) || getLanguage() || getBrowserLanguage();
    };
});
